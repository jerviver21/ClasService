...
create or replace function aud_pedido_func() returns trigger as $$
	begin
		IF TG_OP = 'DELETE' THEN
			INSERT INTO aud_pedido (usuario, cod_pago, valor_total, id_estado, cod_confirmacion, id_entidad_pago, fecha_hora_pago, dni_cliente, nombre_cliente, fecha_vencimiento, operacion, usuario_aud)
			VALUES (OLD.usuario, OLD.cod_pago, OLD.valor_total, OLD.id_estado, OLD.cod_confirmacion, OLD.id_entidad_pago, OLD.fecha_hora_pago, OLD.dni_cliente, OLD.nombre_cliente, OLD.fecha_vencimiento, TG_OP, current_user);
		END IF; 
		IF (TG_OP = 'INSERT' OR  TG_OP = 'UPDATE') THEN
			INSERT INTO aud_pedido (usuario, cod_pago, valor_total, id_estado, cod_confirmacion, id_entidad_pago, fecha_hora_pago, dni_cliente, nombre_cliente, fecha_vencimiento, operacion, usuario_aud)
			VALUES (NEW.usuario, NEW.cod_pago, NEW.valor_total, NEW.id_estado, NEW.cod_confirmacion,  NEW.id_entidad_pago, NEW.fecha_hora_pago, NEW.dni_cliente, NEW.nombre_cliente, NEW.fecha_vencimiento, TG_OP, current_user);
		END IF;	
	return null;
end; $$ language plpgsql;
